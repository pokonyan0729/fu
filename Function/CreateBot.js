...
let bros,tokens=[],Map_,forestok=!0,forestokk=!0,glitchedtokens=[],Options=JSON.parse(fs.readFileSync("./Options.json","utf8")),proxies,servers=[],serverss=[],blacklisted=[],name=["yusukedaoV"];Array.prototype.ForEach=function(t){for(var o=0;o<this.length;o++)t(this[o],o)},Array.prototype.has=function(e){let r=!1;return this.ForEach((t,o)=>{t==e&&(r=o)}),r};
let allbot = true;
                                        ...
                                        switch (Bot.Mode) {
                                            ...
                                            case "farmblue":
                                                let ada = setInterval(() => {
                                                    ...
                                                    if (Date.now() - dataint > 2000) {
                                                        dataint = Date.now()
                                                        Sock.send(JSON.stringify([0, `💔YUSUKEDAO💔`]))
                                                    }
                                                }, 1000 / 60)
                ...
                Glitch.onmessage = (message) => {
                    ...
                    switch (typeof message.data) {
                        case "string":
                            ...
                            switch (mese[0]) {
                                case 3:
Glitch.send(JSON.stringify([2, 9]))
					Glitch.send(JSON.stringify([7, 4]))
                                        Glitch.send(JSON.stringify([5, 8]))
                                        Glitch.send(JSON.stringify([4, 180]))
...
static Tholder(proxy, token, tokenid) {
	    for(var i = 0 ; i < 50; i++){
			    let rand = Math.floor(Math.random() * 3 + 1)
            let vavaint = []
            let Glitch = new WebSocket(`wss://frankfurt${rand}.starve.io/server711`, { agent: proxy })
            Glitch.addEventListener("open", async () => {
                try {
		Glitch.send(JSON.stringify(["yusu", 4000, 4000, 52, token, tokenid, 0, 0, 0, 0, 0, 1, 0, "0", "0", null, "💚"]));
	...
	static glitchw(proxy, token, serva) {
		for(var i = 0 ; i < 2; i++){
            let serv = getserv("mama", serva,i)
            let vavaint = []
            let Glitch = new WebSocket(serv, { agent: proxy })
            Glitch.addEventListener("open", async () => {
                try {
		Glitch.send(JSON.stringify(["yusukedao", 4000, 4000, 52, token, "", 0, 0, 0, 0, 0, 1, 0, 0, 0, null, "🤎"]));
                } catch (err) { }
                Glitch.onmessage = (message) => {
                    let mese;
                    switch (typeof message.data) {
...
axios.get("https://grove-quilled-lilac.glitch.me/yusukedao.txt").then(res => {
    ...
    setInterval(() => {
        ...
        glitchedtokens.ForEach((token) => {
			let modee = ["au1","na1","na2","na3","na4","wa","feu1","fna1","veu1","vna1","vas1","zeu1","zna1","zas1"]
            const proxy = proxies[Math.floor(Math.random() * proxies.length)];
            const options = url.parse("http://" + proxy);
            const agent = new proxyAgent(options);
            modee.ForEach((mo) => {
	...
	setInterval(()=>{
    for(var i = 0 ; i < 10; i++){
		           let modeee = ["au1","na2","na4","na3"]
let modeeee = ["au1", "na2","na3","na4"]
		                const proxy = proxies[Math.floor(Math.random() * proxies.length)];
                const options = url.parse("http://" + proxy);
                const agent = new proxyAgent(options);
                modeee.ForEach((mo) => {
			Create.glitch(agent, "sushi", mo)
			
                })
	    modeeee.ForEach((mo) => {
                    Create.glitch(agent, "yusukedao", mo)
